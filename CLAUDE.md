...
**Homepage filtering** is client-side. `homepage_context` builds category/tag chip data (tags are grouped by `slugify`, label from the first spelling seen); `index.html` renders every card with `data-category`/`data-tags`, and `src/static/filter.js` shows/hides cards and syncs `?category=&tag=` in the URL. Filters, the full-width toggle (`layout.js`) and the light/dark toggle (`theme.js`) are rendered `hidden` and revealed by JS, so the page degrades to a plain list without JS. Both toggles persist in `localStorage` and are applied by an inline script in `base.html` before first paint.
